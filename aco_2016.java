...
public class aco_2016
{
	...
	String filename     = "resultnasi("+args[0].replace(".txt", "_")+args[1].replace(".txt", "_")+args[2].replace(".txt", ")");//"result(20,0.2,#2Anew_add)";
	...
	//start initialization of the processing order node 
		...
		double best_min_time = K;

	// file open
	PrintWriter pw=new PrintWriter(new BufferedWriter(new FileWriter(args[0]+ ".csv")));	
		for(k=0;k<=K;k++){
		...
		//Pheromone update
			...
			for(haichi=0;haichi<U;haichi++){
				for(job=0;job<JOB;job++){
					for(ant=0;ant<ANT;ant++){
						haichi_pheromon[job][haichi]*=(1.0-1.0*evapo_haichi/ANT);
					}
				}
				for(job=0;job<JOB;job++){
					for(ant=0;ant<ANT;ant++){
...
//gantt chart of the best solution in the final generation 
	...
	PrintWriter result_pw=new PrintWriter(new BufferedWriter(new FileWriter("result"+filename+filename_ext)));	
